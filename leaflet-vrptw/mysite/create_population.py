# Code di bawah UDAH Di include di vrptw_genetic_algorithm.py
import os
import math
...
fleets_depot_coord = {
    "XCOORD.": df_fleets.loc[0, "fleet_start_x_coord"],
    "YCOORD.": df_fleets.loc[0, "fleet_start_y_coord"],
}
distance_matrix = []
...
def euclidean_distance(p1, p2):
    return math.sqrt(
        (p1["XCOORD."] - p2["XCOORD."]) ** 2 + (p1["YCOORD."] - p2["YCOORD."]) ** 2
    )
...
def is_route_valid(route, distance_matrix, customers, fleet_total_time, fleet_capacity):
    ...
    for i in range(
        1, len(route) - 2
    ):  # route exclude depot(0), exclude last city sebelum depot
        ...
        if curr_time > due_time or curr_demand > fleet_capacity:
            ...
            if (
                curr_time + wait_time < ready_time
                or curr_time + wait_time + service_time > fleet_total_time
            ):
                route_valid = False
                break
            else:
    ...
    customers.append(
        {
            ...
            "service_time": df_customers.loc[i, "DUE DATE"]
            - df_customers.loc[i, "READY TIME"],
            "ready_time": df_customers.loc[i, "READY TIME"],
            "due_time": df_customers.loc[i, "DUE DATE"],
            "complete_time": df_customers.loc[i, "DUE DATE"]
            + (df_customers.loc[i, "DUE DATE"] - df_customers.loc[i, "READY TIME"]),
        }
    )
...
for experiment in range(experiments):
    population_route_file = os.path.join(
        "./solomon_benchmark/R1/experiments/R101_population_route_"
        + str(experiment)
        + ".csv"
    )
    population_distance_matrix_file = os.path.join(
        "./solomon_benchmark/R1/experiments/R101_population_distance_matrix_"
        + str(experiment)
        + ".csv"
    )
    population_results_file = os.path.join(
        "./solomon_benchmark/R1/experiments/R101_population_results_"
        + str(experiment)
        + ".csv"
    )
